...
import { EMPLOYMENT_LABEL, type EmploymentType, type Job } from "@/lib/types";
import { adminUpdateJob, type ActionState } from "@/lib/actions";
...
const JOB_CATEGORIES = [
  ...
  "영업관리", "영업지원", "QA", "AI서비스기획", "사업기획운영", "콘텐츠 크리에이터",
  "AI리서치엔지니어", "생산", "공정관리", "시스템반도체", "HW엔지니어", "사무직",
];
const REGIONS = ["성남", "판교", "용인", "수원", "서울", "기타"];

export default function AdminJobEditForm({ job, companyName }: { job: Job; companyName: string }) {
  const [skills, setSkills] = useState<string[]>(job.requiredSkills);
  const [customSkill, setCustomSkill] = useState("");
  const [state, formAction, pending] = useActionState<ActionState, FormData>(adminUpdateJob, {});
  ...
  return (
    <div className="mx-auto max-w-2xl px-8 py-12">
      <div className="flex items-center justify-between mb-1">
        <h1 className="text-xl font-bold">공고 수정</h1>
        <Link href="/admin/jobs" className="text-sm text-muted hover:text-indigo">← 공고 관리</Link>
      </div>
      <p className="text-sm text-muted mb-8">{companyName}의 공고를 수정합니다.</p>
      ...
      <form action={formAction} className="space-y-7">
        ...
        <div>
          <label className="text-sm font-semibold block mb-1.5">회사</label>
          <p className="w-full rounded-xl border border-line bg-gray-50 px-4 py-3 text-sm text-muted">{companyName}</p>
        </div>

        <div>
          <label className="text-sm font-semibold block mb-1.5">공고 제목 <span className="text-indigo">*</span></label>
          <input name="title" required defaultValue={job.title} placeholder="예) CNC 가공 엔지니어 (신입)"
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            ...
            <select name="jobCategory" required defaultValue={job.jobCategory}
              className="w-full rounded-xl border border-line px-4 py-3 text-sm focus:outline-none focus:border-indigo">
        ...
        <div>
          ...
          <select name="region" defaultValue={job.region}
            className="w-full rounded-xl border border-line px-4 py-3 text-sm focus:outline-none focus:border-indigo">
        ...
        <div>
          ...
          <input name="postingUrl" defaultValue={job.postingUrl ?? ""} placeholder="https://"
            className="w-full rounded-xl border border-line px-4 py-3 text-sm focus:outline-none focus:border-indigo" />
        </div>
