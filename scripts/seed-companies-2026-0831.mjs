// 2026 IT·AI 기업매칭 — 추가 확정 기업 2곳 계정 생성 (0827 배치 이후 추가분).
//
// 실행 (셋 중 아무거나):
//   node scripts/seed-companies-2026-0831.mjs
//   node scripts/seed-companies-2026-0831.mjs --url=... --key=...
//   NEXT_PUBLIC_SUPABASE_URL=... SUPABASE_SERVICE_ROLE_KEY=... node scripts/seed-companies-2026-0831.mjs
//
...
/** 표에 없는 항목(업종·지역·전화번호·수요인원·우선순위)은 비워 둔다 — 추정해서 넣지 않는다. */
const COMPANIES = [
  { name: "이한크리에이티브", contactName: "전혜란", contactTitle: "대표",
    email: "orange@ehancreative.com", fields: ["SW/앱개발", "AI/데이터/LLM"], status: "확정" },
  { name: "지티엠케이(GTMK)", contactName: "김기만", contactTitle: "전무",
    email: "keyman@gtmk.co.kr", fields: ["SW/앱개발", "AI/데이터/LLM"], status: "확정" },
];
...
const hasContacts = await (async () => {
  ...
  if (!error) return true;
  console.warn("! company_contacts 테이블이 없어 담당자·수요조사 정보는 저장하지 않습니다.\n");
  return false;
})();
...
if (!hasContacts) {
  console.log("\n※ 담당자·수요조사 정보는 저장하지 않았습니다(company_contacts 테이블 없음).");
}
